...
- Strip: 32 px tall, 44 px hit area (`::after`), portrait `top: var(--prompt-h) + 8px`, `left: 8px`, `right: 100px` (full width while expanded); landscape bottom-left 236 px wide (284 px expanded).
...
- Prompt: the `short` body is used on both phone layouts (it was portrait-only) and the prompt re-renders when the layout class changes; "click" reads "tap" on `(pointer: coarse)`. `nameBuilding` no longer says "click the roof sign" (the choices open by themselves).
