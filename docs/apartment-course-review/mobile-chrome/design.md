...
- Framing (portrait): `FOOTPRINT` per type (width, depth, fill: plex 14×9 @0.85, tower 18×9 @0.84, row 24×9 @0.78, block 16×10 @0.84, seq 16×26 @0.75); `portraitPos` backs off along the pose's view ray until the projected width fills that share of the screen, adds the near face's lead for straight-on views, never nearer than 0.75× the desktop distance. Result at 375×812: the 4-plex spans ~80 % of the width (was ~50 %), the Part 4 block fills the width with readable five-digit plates (was 34 %), the Part 3 row lands where the old 1.55× constant put it. The Part 5 walk-around views (`setSeqView`) fill 0.88 instead (`SEQ_VIEW_FILL`): one face to read, and the side views' camera then stays in front of the neighbours the Sunbelt and Suburban looks stand beyond the ends of the building (at 0.75 it sat on the Sunbelt neighbour's roof, which filled the lower half of the screen).
...
- Touch: tap slop 12 px for `pointerType: touch`; a tap that misses every plate opens the nearest unsolved plate within 32 px (`nearestBlank`); halos are a fixed frame round the plate (1.16 × 1.34) on every screen — the 48 px floor this pass added was dropped in September 2026 (see the decisions below); the opened plate tints for 700 ms; the sheet's subline repeats the plate's partial number ("The plate shows 71▢5 — pick the full number").
...
- Halo: a fixed 1.16 × 1.34 frame on every screen. This pass gave it a 48 px floor on coarse pointers (the plates are 1.7-2 units wide, so a fixed factor looked too small at the row/seq distances), but on a phone the floor made the glow two to three times its plate on Parts 2-5 while Part 1's stayed snug, so it was dropped in September 2026 and every building now matches Part 1. The tap target was never the glow: the plate / door mesh and the 32 px snap carry a touch.
